...
public class AABubbleTextCell : AABubbleCell, TTTAttributedLabelDelegate {
    ...
    public func openUrl(url: NSURL) {
        if url.scheme == "source" {
            ...
            self.controller.navigateDetail(AACodePreviewController(code: code))
        } else {
            //ActorSDK.sharedActor().openUrl(url.absoluteString)
            let browser = TOWebViewController(URLString: url.absoluteString)
            browser.showUrlWhileLoading = false
            self.controller.navigateDetail(browser)
        }
    }
