...
object UserErrors {
  ...
  case object NicknameTaken extends UserError("昵称已经被他人使用")

  final case class InvalidTimeZone(tz: String) extends UserError(s"无效市区: $tz")
    ...
    p.contact.UnregisteredPhoneContactRepo.find(phoneNumber) flatMap { contacts ⇒
      ...
      val actions = contacts map { contact ⇒
        ...
        for {
          ...
          _ ← DBIO.from(userExt.broadcastUserUpdate(contact.ownerUserId, updateContactRegistered, Some(s"${localName.getOrElse(user.name)} 已注册"), isFat = true, deliveryId = None))
