    ...
    for {
      ...
      _ ← Future.sequence(contacts map { contact ⇒
        ...
        for {
          ...
          _ ← userExt.broadcastUserUpdate(contact.ownerUserId, updateContactRegistered, Some(s"${localName.getOrElse(user.name)} 已注册"), isFat = true, reduceKey = None, deliveryId = None)
